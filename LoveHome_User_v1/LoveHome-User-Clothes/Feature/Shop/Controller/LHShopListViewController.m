...
- (void)setupVar {
    ...
    _secondDataArry = @[@"离我最近",@"信用优先",@"销量优先"];
    ...
    switch (mode) {
        ...
        case JXWebLaunchModeRefresh: {
            ...
            [self.operaters exAddObject:
             [LHHTTPClient requestGetActivityShopsWithActivityid:_activity.activityid baseProductId:_activity.baseProductId lng:gLH.longitude lat:gLH.latitude range:[self getCurDistance] sort:[self getCurSort] page:1 rows:kPageSize success:^(AFHTTPRequestOperation *operation, LHActivityShopList *shopList) {
        ...
        case JXWebLaunchModeMore: {
            [self.operaters exAddObject:
             [LHHTTPClient requestGetActivityShopsWithActivityid:_activity.activityid baseProductId:_activity.baseProductId lng:gLH.longitude lat:gLH.latitude range:[self getCurDistance] sort:[self getCurSort] page:(self.curPage + 1) rows:kPageSize success:^(AFHTTPRequestOperation *operation, LHActivityShopList *shopList) {
...
- (void)requestMapShopsWithMode:(JXWebLaunchMode)mode {
    ...
    [LHHTTPClient queryShopsWithLatitude:gLH.latitude longitude:gLH.longitude type:self.from distance:100000.0 sort:[self getCurSort] index:1 size:10000 success:^(AFHTTPRequestOperation *operation, LHShopList *shopList) {
...
- (void)requestShopsWithMode:(JXWebLaunchMode)mode {
    switch (mode) {
        case JXWebLaunchModeLoad: {
            [JXLoadView showProcessingAddedTo:self.tableView rect:self.tableRect];
            [self.operaters exAddObject:
             [LHHTTPClient queryShopsWithLatitude:gLH.latitude longitude:gLH.longitude type:self.from distance:[self getCurDistance] sort:[self getCurSort] index:1 size:_page.pageSize success:^(AFHTTPRequestOperation *operation, LHShopList *shopList) {
        ...
        case JXWebLaunchModeMore: {
            [self.operaters exAddObject:
             [LHHTTPClient queryShopsWithLatitude:gLH.latitude longitude:gLH.longitude type:self.from distance:[self getCurDistance] sort:[self getCurSort] index:(self.page.currentPage + 1) size:self.page.pageSize success:^(AFHTTPRequestOperation *operation, LHShopList *shopList) {
