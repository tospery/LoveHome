    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                ...
                float myFixed = self.segmentWidthsArray.count == 0 ? 0.0 : [[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue];
                return CGRectMake(selectedSegmentOffset + self.selectionIndicatorEdgeInsets.left,
                                  indicatorYOffset,
                                  myFixed - self.selectionIndicatorEdgeInsets.right,
...
- (CGRect)frameForFillerSelectionIndicator {
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
        ...
        CGFloat myFixed = self.segmentWidthsArray.count == 0 ? 0.00f : [[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue];
        return CGRectMake(selectedSegmentOffset, 0, myFixed, CGRectGetHeight(self.frame));
    }
...
- (void)scrollToSelectedSegmentIndex:(BOOL)animated {
    ...
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        float myFixed = self.segmentWidthsArray.count == 0 ? 0.00f : [[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue];
        rectForSelectedIndex = CGRectMake(offsetter,
                                          0,
                                          myFixed,
                                          self.frame.size.height);
        
        selectedSegmentOffset = (CGRectGetWidth(self.frame) / 2) - (myFixed / 2);
    }
