...
- (void)viewDidLoad {
   ...
   /* if (_type == OrderTypeAdded || _type == OrderTypehandled) { */
        _tooBr = [[NSBundle mainBundle] loadNibNamed:@"RecctAndRejectToolBar" owner:nil options:nil].lastObject;
        __weak __typeof(self)weakSelf = self;
        
      /*  if (self.orderDetail.status == 12 || self.orderDetail.status == 2) { */
#pragma mark - 只是暂时写为1 , 具体的再改
          
//            if (self.orderDetail.collectedByMerchant == 1) {
...
//                [_tooBr.rightButton setBackgroundColor:JXColorHex(0xff4400)];
//                _tooBr.leftButton.hidden = YES;
//            }
//            else
//            {
    if (_type == OrderTypeAdded) {
        
        
        if (self.orderDetail.collectedByMerchant != 0) {
            
            _tooBr.rightButton.enabled = NO;
            _tooBr.rightButton.backgroundColor = [UIColor lightGrayColor];
            [_tooBr.rightButton setTitleColor:[UIColor whiteColor] forState:UIControlStateDisabled];
            _tooBr.rightButton.layer.borderColor = [UIColor lightGrayColor].CGColor;
            
        } else {
            
            [_tooBr.rightButton setTitle:@"确认收件" forState:UIControlStateNormal];
        }

    }
    if (_type == OrderTypehandled || _type == OrderTypeFinished || _type == OrderTypeRejected) {
        
        [_tooBr.leftButton mas_remakeConstraints:^(MASConstraintMaker *make) {
           
            make.top.mas_equalTo(_tooBr).offset(15);
            make.left.mas_equalTo(_tooBr).offset([UIScreen mainScreen].bounds.size.width - 75 - 15);
            make.right.mas_equalTo(_tooBr).offset(-15);
            make.width.mas_equalTo(75);
            make.height.mas_equalTo(30);
        }];
        NSLog(@"%@", _tooBr.leftButton);
        _tooBr.rightButton.hidden = YES;
    }
                _tooBr.rightClick = ^()
                {
...
#pragma mark - 同样替换为打电话
                    NSLog(@"打电话了1");
                    [weakSelf makeCall:weakSelf.orderDetail];
                };
...
//                NSLog(@"测试收衣服");
//#pragma mark - 接口更换完毕
//                [OrderTools confirmOrderClothes:strongSelf.orderDetail.orderid success:^(AFHTTPRequestOperation *operation, NSNumber *result) {
//                    ShowProgressHUD(NO, nil);
//                    strongSelf.tooBr.hidden = YES;
//                    ShowWaringAlertHUD(@"确定收衣成功", nil);
//                    if (strongSelf.rightActionBlcok) {
//                        strongSelf.rightActionBlcok(strongSelf.orderDetail,strongSelf.type);
//                    }
//                    
...
//                }
//             
//#pragma mark - 此处点击事件被替换为拨打电话
//                NSLog(@"打电话了2");
//
//                [strongSelf makeCall:strongSelf.orderDetail];
//            };
//        }
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    if (section == 1) {
        ...
        if (![_orderDetail.remark  isEqual: @""]) {
            
        
        view.remarkContent.text = [NSString stringWithFormat:@"客户留言: %@", _orderDetail.remark];
        } else {
           view.remarkContent.text = @"客户无留言";
        }
        NSLog(@"%@", _orderDetail.remark);
...
- (void)initNavBar
{
    ...
    titleLable.backgroundColor = JXColorHex(0xfc9a7a);
