...
+ (UIImage *)cutImageByHeightScale:(CGFloat)heightScale andOrginImage:(UIImage *)orginImage
{
    if (orginImage)
    {
        ...
        //绘制原图
        CGSize originImageSize = CGSizeMake(orginImage.size.width, orginImage.size.height);
        
        width  = originImageSize.width;
        height = originImageSize.width*heightScale;
        
        //计算出的高度，小于图片的高度(图片高度大于预计高度，需要做竖直方向居中裁剪)
        if (height < originImageSize.height)
        {
            y = fabsf(originImageSize.height-height)/2.0f;
            
        }else if(height > orginImage.size.height) //计算出的高度，大于图片的实际高度（图片高度小于预计高度，需要做横向的居中裁剪）
        {
            width  = originImageSize.height/heightScale;
            height = originImageSize.height;
            
            x = fabsf(originImageSize.width-width)/2.0f;
        }
        
        
        CGRect subImageRect    = CGRectMake(x, y, width, height);
        CGImageRef imageRef    = orginImage.CGImage;
        CGImageRef subImageRef = CGImageCreateWithImageInRect(imageRef, subImageRect);
        UIImage *subImage      = [UIImage imageWithCGImage:subImageRef];
        CGImageRelease(subImageRef);
...
{
    NSData *data = [UIImage imageObjectToData:originImage andCompressionQuality:1.0 andMaxSize:500];
