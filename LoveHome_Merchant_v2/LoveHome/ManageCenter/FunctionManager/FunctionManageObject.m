...
void ShowIndicatorView(BOOL toShow,id toView)
{
    UIView *currentView = (UIView *)toView;
    
    if (toShow) {
        
        UIActivityIndicatorView *view = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
        view.center = currentView.center;
...
#pragma mark - 适配相关
UIFont *RHFontAdap(CGFloat size)
{

    return [UIFont systemFontOfSize:size * (SCREEN_WIDTH/320)];
}

CGFloat RHHeightAdap(CGFloat height)
{
     return height  *(SCREEN_HIGHT/568);
}
CGFloat RHWidthAdap(CGFloat width)
{
     return width  *(SCREEN_WIDTH/568);
}
...
void ShowWaringAlertHUD(NSString *message, id toView)
{
    if (message && [message length] > 0)
    {
        if (toView)
        {
            [MBProgressHUD showWaring:message toView:toView];
        }else
        {
            [MBProgressHUD showWaring:message toView:[UIApplication sharedApplication].keyWindow];
        }
    }
...
UIViewController* getcurrentController(void)
{
...
void ConfigButtonLayerBorderColor(UIColor *borderColor,UIButton *button)
{
