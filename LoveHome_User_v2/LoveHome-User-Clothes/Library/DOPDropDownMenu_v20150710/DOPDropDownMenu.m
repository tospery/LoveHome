...
@property (nonatomic, copy) NSArray *middleSeperators;
@end
...
//    CGFloat separatorLineInterval = self.frame.size.width / _numOfMenu;
    ...
    NSMutableArray *tempMiddleSeperators;
    if (!_hideMiddleSeperator) {
        tempMiddleSeperators = [[NSMutableArray alloc] initWithCapacity:_numOfMenu];
    }
    
    for (int i = 0; i < _numOfMenu; i++) {
        if (!_hideMiddleSeperator) {
            if (i != (_numOfMenu - 1)) {
                UIView *middleLineView = [[UIView alloc] initWithFrame:CGRectMake(bgLayerInterval*(i+1), 10.0f, 0.5f, self.bounds.size.height-20.0f)];
                middleLineView.backgroundColor = [UIColor colorWithRed:205.0f/255.0f green:205.0f/255.0f blue:205.0f/255.0f alpha:1.0];
                [self addSubview:middleLineView];
                [tempMiddleSeperators addObject:middleLineView];
            }
        }
    ...
    if (!_hideMiddleSeperator) {
        _middleSeperators = [tempMiddleSeperators copy];
    }
}
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(0, 0, self.frame.size.width/self.numOfMenu - 1, self.frame.size.height-1);
...
- (CAShapeLayer *)createIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    if (DOPDropDownMenuSeparatorTypeTriangle == _separator) {
    ...
    }else if (DOPDropDownMenuSeparatorTypeArrow == _separator) {
        CGMutablePathRef path =  CGPathCreateMutable();
        CGPathMoveToPoint(path, NULL, 0, 0);
        CGPathAddLineToPoint(path, NULL, 5, 6);
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    if (tapIndex == _currentSelectedMenudIndex && _show) {
        ...
        if (!_hideMiddleSeperator) {
            [self configMiddleSeperatorHide:NO index:tapIndex];
        }
    } else {
        _currentSelectedMenudIndex = tapIndex;
        ...
        if (!_hideMiddleSeperator) {
            [self configMiddleSeperatorHide:YES index:tapIndex];
        }
    }
}
...
- (void)backgroundTapped:(UITapGestureRecognizer *)paramSender
{
    ...
    if (!_hideMiddleSeperator) {
        [self configMiddleSeperatorHide:NO index:_currentSelectedMenudIndex];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.textLabel.text == [(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]) {
        ...
        cell.textLabel.textColor = [UIColor colorWithRed:0.0f/255.0f green:123.0f/255.0f blue:255.0f/255.0f alpha:1.0];
    }else {
...
- (void)configMenuWithSelectRow:(NSInteger)row {
    UITableViewCell *cell = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:_currentSelectedMenudIndex inSection:0]];
    cell.accessoryType = UITableViewCellAccessoryNone;
    cell.textLabel.textColor = [UIColor blackColor];
    cell = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:0]];
    cell.accessoryType = UITableViewCellAccessoryCheckmark;
    cell.textLabel.textColor = [UIColor colorWithRed:0.0f/255.0f green:123.0f/255.0f blue:255.0f/255.0f alpha:1.0];
...
#pragma mark - extend
- (void)configMiddleSeperatorHide:(BOOL)hide index:(NSInteger)index {
    if (!_middleSeperators) {
        return;
    }
    
    if (index < 0 || index > _middleSeperators.count) {
        return;
    }
    
    NSInteger count = _middleSeperators.count;
    if (0 == index) {
        [_middleSeperators[0] setHidden:hide];
    }else if (index == count) {
        [_middleSeperators[count - 1] setHidden:hide];
    }else {
        [_middleSeperators[index] setHidden:hide];
        [_middleSeperators[index - 1] setHidden:hide];
    }
}
@end
