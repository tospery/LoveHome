    ...
    if (_isButtonPressed) {
        if (_text.length > 0){
            if (JXCollapsibleViewModeClose == _mode) {
                ...
                _textWidth = self.bounds.size.width - kJXStandardPadding * 2 - JXCollapsibleViewButtonEdge;
                CGSize requiredSize = [_text exSizeWithFont:_font width:_textWidth];
                _matchHeight = requiredSize.height + kJXStandardPadding;
            }else if (JXCollapsibleViewModeOpen == _mode) {
                _mode = JXCollapsibleViewModeClose;
                _matchHeight = _baseHeight * _lines + kJXStandardPadding;
            }
        }else {
            _matchHeight = 0;
        }
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_text.length > 0) {
        ...
        CGRect textFrame = CGRectMake(kJXStandardPadding,
                                      kJXStandardPadding / 2.0,
                                      textWidth,
                                      _matchHeight - kJXStandardPadding / 2.0);
