...
- (void)productButtonPressed:(UIButton *)btn {
    if (_isOutOfService) {
        if (self.selectBlock) {
            // (LHProduct *product, LHSpecify *specify, UIButton *btn, BOOL select);
            self.selectBlock(self.product, nil, btn, btn.selected, self.logoImageView);
        }
        return;
    }
...
//
            ...
            if (self.selectBlock) {
                self.selectBlock(self.product, s, btn, btn.selected, self.logoImageView);
            }
            break;
        }
    }
...
- (void)configView {
    ...
    [self.logoImageView sd_setImageWithURL:[NSURL URLWithString:_product.url] placeholderImage:kImagePHProductIcon];
    ...
    for (LHSpecify *s in _product.specifies) {
